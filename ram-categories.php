...
switch ($sortOption) {
    case 'price_low_to_high':
        $order = "ORDER BY ram_price ASC";
        break;
    case 'price_high_to_low':
        $order = "ORDER BY ram_price DESC";
        break;
    case 'newest_first':
        $order = "ORDER BY ram_date DESC";
        break;
    case 'oldest_first':
        $order = "ORDER BY ram_date ASC";
        break;
    case 'highest_rated':
    ...
    case 'cpu_performance':
        ...
        break;
    case 'ram_size':
        $order = "ORDER BY ram_capacity DESC";
        break;
    case 'brand':
        $order = "ORDER BY ram_brand ASC";
        break;
    default:
        $order = "ORDER BY ram_price ASC";
}
...
<head>
    ...
    <script>
        function applySort() {
            ...
            window.location.href = 'ram-categories.php?sort=' + sort;
        }
    </script>
</head>
<body>
        ...
        <div class="container">
            <div class= "even-flex padding-block">
                <div class="path fw-light fs-normal text-neutral">
                    <a href="homepage.php">Home</a> > <a href="homepage.php">Categories</a> > <div class="fw-bold">Random Access Memory</div>
                </div>
                ...
                <div class="sort-all">
                    ...
                    <div class="custom-select-wrapper">
                        <select name="sort" id="sort" class="custom-select" onchange="applySort()">
                            ...
                            <option value="oldest_first" <?php if ($sortOption == 'oldest_first') echo 'selected'; ?>>Oldest First</option>
                            ...
                            <option value="ram_size" <?php if ($sortOption == 'ram_size') echo 'selected'; ?>>RAM Size</option>
            ...
            <?php foreach ($rows as $row) : ?>
                <div class="ram-card">
                    <div class="ram-card-content">
                        <h2 class="ram-card-title fw-bold text-neutral fs-normal"><?php echo htmlspecialchars($row['ram_name']); ?></h2>
                        <img src="img/<?php echo htmlspecialchars($row['ram_image']); ?>" alt="<?php echo htmlspecialchars($row['ram_name']); ?>" class="ram-card-image">
                        <div class="ram-card-detail">
                            <p class="text-neutral fw-regular">
                                Ram Capacity: <?php echo htmlspecialchars($row['ram_capacity']); ?>GB
                            </p>
                            <p class="text-neutral fw-regular">
                                Data Rate: <?php echo htmlspecialchars($row['ram_data_rate']); ?>MHz
                            </p>
                            <p class="text-neutral fw-regular">
                               Voltage: <?php echo htmlspecialchars($row['ram_voltage']); ?>V
                            </p>
                            <p class="text-neutral fw-regular">
                                Timing: <?php echo htmlspecialchars($row['ram_timing']); ?>
                            </p>
                            <p class="text-neutral fw-regular">
                                Brand: <?php echo htmlspecialchars($row['ram_brand']); ?>
                            </p>
                            <p class="text-neutral fw-regular">
                                Date Released: <?php echo htmlspecialchars($row['ram_date']); ?>
                            </p>
                            <p class="text-neutral fw-regular">
                                Price: PHP <?php echo htmlspecialchars(number_format($row['ram_price'], 2)); ?>
                            </p>
                        </div>
